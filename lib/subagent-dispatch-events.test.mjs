...
test("P2: AbortSignal handler is detached after settle (abort after settle is harmless)", async () => {
  ...
  // Abort the signal AFTER settle — must be a no-op (no crash, no second settle).
  ac.abort();
  // The completion promise must still resolve with "completed", not "aborted".
...
test("P2: AbortSignal listener removed when start() throws", async () => {
  ...
  // Abort after the failed start must be harmless (no lingering listener).
  ac.abort(); // Must not throw or trigger any side effects.
  assert.ok(true, "abort after failed start must be harmless");
});
