...
test("S6: ephemeral dispatch creates no .jsonl file", async () => {
  ...
  try {
    ...
    // The dispatch module must forward ephemeral: true on the request.
    assert.equal(controller._calls.start[0].ephemeral, true,
      "dispatch must forward ephemeral:true to the controller");
...
test("S6: non-ephemeral dispatch forwards ephemeral:false and performs no self-built persistence", async () => {
  ...
  try {
    ...
    // The dispatch module must forward ephemeral: false on the request.
    assert.equal(controller._calls.start[0].ephemeral, false,
      "dispatch must forward ephemeral:false to the controller");

    // The dispatch module performs no self-built persistence (B6 transfer):
    // no .jsonl should appear under the injected root at the dispatch level.
    const sessions = await SessionManager.list(root);
    assert.equal(sessions.length, 0,
      "dispatch module must not self-build persistence after B6 transfer");
  } finally {
    await rm(root, { recursive: true, force: true }).catch(() => {});
  }
});
